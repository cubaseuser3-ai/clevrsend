<template>
  <canvas ref="canvasRef" class="dark-veil-canvas"></canvas>
</template>
...
onMounted(() => {
  ...
  const loop = (currentTime: number) => {
    ...
    (program.uniforms.uTime as any).value =
      ((currentTime - start) / 1000) * props.speed;
    (program.uniforms.uHueShift as any).value = props.hueShift;
    (program.uniforms.uNoise as any).value = props.noiseIntensity;
    (program.uniforms.uScan as any).value = props.scanlineIntensity;
    (program.uniforms.uScanFreq as any).value = props.scanlineFrequency;
    (program.uniforms.uWarp as any).value = props.warpAmount;
